...
| `engine/`     | The `SyncEngine` trait (`mod.rs`) and its implementations: `RcloneEngine` (`rclone.rs`), an HTTP-based engine (`http.rs`), rclone binary resolution/download (`rclone_path.rs`), staging dirs (`staging.rs`). |
