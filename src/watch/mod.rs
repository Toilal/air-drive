...
impl Watcher {
    ...
    /// channel; the daemon's debouncer reads from it.
    ///
    ...
    pub fn start(local_root: &Path) -> Result<(Self, mpsc::Receiver<WatchEvent>)> {
        ...
        let handler = move |res: notify::Result<notify::Event>| {
            ...
            for converted in convert_event(&ev, &handler_root) {
...
fn convert_event(ev: &notify::Event, root: &Path) -> Vec<WatchEvent> {
    let mut out = Vec::new();
...
/// per FR-013.
fn accept_local_file(path: &Path) -> bool {
...
mod tests {
    ...
    async fn watcher_emits_created_for_new_file() {
        ...
        let (_w, mut rx) = Watcher::start(tmp.path()).unwrap();
